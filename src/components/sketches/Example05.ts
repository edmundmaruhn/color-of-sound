...
export class Example05 extends Sketch {
	...
	setup = () => {
		...
		this.sequence = new Sequence(this, { stroke: { thickness: 3 }, joint: { radius: 21 }, close: false })
	...
	draw = () => {
		...
		const audio = this.getAudioInfo(1, 4)

		//audio.frequencies = audio.frequencies.map((frequency, index) => (index % 2 === 0 ? frequency * 2 : frequency))
		//console.log(energies.toArray())
		...
		const config = { scale: 0.1, osc, audio, samples, path: this.polygon }
	...
	getAudioInfo = (min: number = 0, max: number = 1): AudioInfo => {
		return {
			frequencies: this.fft.analyze().map((frequency) => this.map(frequency, 0, 255, min, max)),
			energies: {
				...
				toArray: function () {
					return Object.values(this).filter((value) => typeof value === 'number') as number[]
				},
			},
		}
	}
	...
	render = (config: Configuration) => {
		//config.path = Sequence.radialPolygon(...config.audio.frequencies).map((value) => value * 100)
		config.path = Sequence.line(377, 233, ...config.audio.frequencies.map((frequency) => frequency * 2)) //.map((value) => value * 100)
		for (let sample = 0; sample < config.samples; ++sample) {
			config.currentSample = this.frameCount - config.samples + sample
			config.sampleIndex = sample
	...
	renderSample = (config: Required<Configuration>) => {
		...
		this.sequence.render(...config.path)
	...
	applyTransformations = (config: Required<Configuration>) => {
		...
		const alpha = this.map(config.sampleIndex, 0, config.samples - 1, 0, 1)
		const color = this.color(config.currentSample % this.circleSteps, 100, 100, alpha) // * osc
		this.stroke(color)
		this.fill(color)
		...
		this.rotate((this.map(config.currentSample, 0, 30, 0, 1) / 2) * Math.PI)
